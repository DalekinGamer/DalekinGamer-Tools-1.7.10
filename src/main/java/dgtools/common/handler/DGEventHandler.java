...
public class DGEventHandler
{
	...
	{
		if (event.crafting.getItem() == ItemRegistry.ItemSimple && event.crafting.getItemDamage() == 5) giveResearch(event.player);
	}
	...
	public void livingDrop (LivingDropsEvent event)
	{
		if (event.source.getEntity() instanceof EntityPlayer)
		{
			...
			{
				Random r = new Random();
				ItemStack drops = new ItemStack(ItemRegistry.ItemSimple, r.nextInt(5), 7);
