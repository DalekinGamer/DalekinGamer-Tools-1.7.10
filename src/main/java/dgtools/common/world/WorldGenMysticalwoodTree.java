...
public class WorldGenMysticalwoodTree extends WorldGenAbstractTree
{
	public Block trunk;
	public Block leaves;
	...
	public WorldGenMysticalwoodTree (boolean b, Block trunk, Block leaves)
	{
		super(b);
		this.trunk = trunk;
		this.leaves = leaves;
		...
		for (aint1[par5] = aint[par5]; j1 <= i1; ++j1){
			...
			while (k1 <= i1){
				...
				if (d0 > (double)(par4 * par4)){
					...
					if (l1 != Blocks.air && l1 != BlockRegistry.ItemBlockMysticalwoodLeaves){
						++k1;
					}else{
	...
	void generateLeafNode(int par1, int par2, int par3){
		...
		for (int i1 = par2 + this.leafDistanceLimit; l < i1; ++l){
			...
			this.genTreeLayer(par1, l, par3, f, 1, BlockRegistry.ItemBlockMysticalwoodLeaves);
		}
	}
	...
	int checkBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger){
		...
		if (aint2[b1] == 0){
			...
			for (j = aint2[b1] + b4; i != j; i += b4){
				...
				if (k != Blocks.air && k != BlockRegistry.ItemBlockMysticalwoodLeaves){
					break;
				}
			}
	...
	boolean validTreeLocation ()
	{
		...
		boolean isSoil = block.canSustainPlant(worldObj, basePos[0], basePos[1] - 1, basePos[2], ForgeDirection.UP, (BlockSapling) BlockRegistry.ItemBlockMysticalwoodSapling);
		if (!isSoil)
		{
			return false;
		}
