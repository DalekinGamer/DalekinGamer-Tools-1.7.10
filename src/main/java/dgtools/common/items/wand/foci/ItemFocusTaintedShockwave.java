...
public class ItemFocusTaintedShockwave extends ItemFocusBasic
{
	...
	{
		this.icon = ir.registerIcon("dg:ItemFocusTaintedShockwave");
		this.depthIcon = ir.registerIcon("dg:ItemFocusTaint_depth");
		this.ornIcon = ir.registerIcon("dg:ItemFocusTaintedShockwave_orn");
	}
		...
		if (wand.consumeAllVis(s, p, getVisCost(s), true, false))
		{
			...
			if (ents != null && ents.size() > 0)
			{
				for (int a = 0; a < ents.size(); a++)
				{
					...
					{
						double dist = DgToolsHelper.getDistanceTo(e.posX, e.posY, e.posZ, p);
						if (dist < 7.0D) e.attackEntityFrom(DamageSource.magic, 2.0F);
						Vector3 movement = DgToolsHelper.getVectorBetweenEntities(e, p);
						e.addVelocity(movement.x * 3, 0.8, movement.z * 3);
					}
				}
			}
			w.playSoundAtEntity(p, "dg:shockwave", 5.0F, 1.5F * (float) Math.random());
